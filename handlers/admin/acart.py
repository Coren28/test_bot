...
@dp.message_handler(IsAdmin(), text=cart)
...
@dp.callback_query_handler(IsAdmin(), product_cb.filter(action='count'))
@dp.callback_query_handler(IsAdmin(), product_cb.filter(action='increase'))
@dp.callback_query_handler(IsAdmin(), product_cb.filter(action='decrease'))
...
@dp.message_handler(IsAdmin(), text='📦 Оформить заказ')
...
@dp.message_handler(IsAdmin(), text=back_message, state=CheckoutState.name)
async def process_name_back(message: Message, state: FSMContext):
    await CheckoutState.check_cart.set()
    await checkout(message, state)


@dp.message_handler(IsAdmin(), state=CheckoutState.name)
...
@dp.message_handler(IsAdmin(), text=back_message, state=CheckoutState.address)
...
@dp.message_handler(IsAdmin(), state=CheckoutState.address)
...
@dp.message_handler(IsAdmin(), text=back_message, state=CheckoutState.confirm)
...
@dp.message_handler(IsAdmin(), text=confirm_message,
                    state=CheckoutState.confirm)
async def process_confirm(message: Message, state: FSMContext):
